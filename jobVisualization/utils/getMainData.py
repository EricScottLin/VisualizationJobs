...
from .getEducationalCharData import getAverged
    ...
    for i in jobs:
        if i.address in ['唐山', '秦皇岛', '邯郸', '邢台', '保定', '张家口', '承德', '沧州', '廊坊', '衡水', '石家庄']:
            ...
            if addressData.get('西藏', -1) == -1:
                ...
                addressData['西藏'] += 1
        elif i.address in ['兰州', '嘉峪关', '金昌', '白银', '天水', '酒泉', '张掖', '武威', '定西', '陇南', '平凉', '庆阳', '临夏回族自治州',
                           '甘南藏族自治州']:
            ...
            if addressData.get('宁夏', -1) == -1:
                ...
                addressData['宁夏'] += 1
        elif i.address in ['乌鲁木齐', '克拉玛依', '吐鲁番', '哈密', '昌吉回族自治州', '博尔塔拉蒙古自治州', '巴音郭楞蒙古自治州', '阿克苏', '克孜勒苏柯尔克孜自治州', '喀什',
                           '和田', '伊犁哈萨克自治州', '塔城', '阿勒泰']:
...
def getMaxSalary():
    ...
    jobs = getAllJobs()
    for i in jobs:
        x = (json.loads(i.salary)[1]) / 1000
        if x > maxSalary:
            maxSalary = x
    return maxSalary


def getEducationalSalary():
    hasEmpty = False
    jobs = JobInfo.objects.all()
    education = {}
    people = {}
    ...
    for job in jobs:
        for k, v in education.items():
            if job.education == k:
                    education[k].append(json.loads(job.salary)[1])
                    people[k] += 1
    for k, v in education.items():
        try:
            education[k] = getAverged(v)
        except:
            education[k] = 0
    ...
    return educationList, list(education.values()), list(people.values()), hasEmpty
